...
type Controller struct {
	...
	// jobStoreSynced returns true if the VideoAnalyticsJob store has been synced at least once.
	// Added as a member to the struct to allow injection for testing.
	jobStoreSynced cache.InformerSynced

	// A store of jobs
	jobLister sednav1listers.VideoAnalyticsJobLister
...
// enqueueByPod enqueues the VideoAnalyticsJob object of the specified pod.
func (c *Controller) enqueueByPod(pod *v1.Pod, immediate bool) {
	...
	job, err := c.jobLister.VideoAnalyticsJobs(pod.Namespace).Get(controllerRef.Name)
	if err != nil {
		return
	}
...
// When a pod is updated, figure out what VideoAnalytics job manage it and wake them up.
...
// deletePod enqueues the VideoAnalyticsJob obj When a pod is deleted
func (c *Controller) deletePod(obj interface{}) {
	...
	// changed labels the new VideoAnalyticsJob will not be woken up till the periodic resync.
	if !ok {
...
// obj could be an *sednav1.VideoAnalyticsJob, or a DeletionFinalStateUnknown marker item,
...
func (c *Controller) processNextWorkItem() bool {
	...
	klog.Warningf("Error syncing VideoAnalytics job: %v", err)
	c.queue.AddRateLimited(key)

	return true
}

// sync will sync the VideoAnalyticsJob with the given key if it has had its expectations fulfilled, meaning
// it did not expect to see any more of its pods created or deleted. This function is not meant to be invoked
// concurrently with the same key.
func (c *Controller) sync(key string) (bool, error) {
	startTime := time.Now()
	defer func() {
		klog.V(4).Infof("Finished syncing VideoAnalytics job %q (%v)", key, time.Since(startTime))
	}()
	...
	if len(ns) == 0 || len(name) == 0 {
		return false, fmt.Errorf("invalid VideoAnalytics job key %q: either namespace or name is missing", key)
	}
	sharedJob, err := c.jobLister.VideoAnalyticsJobs(ns).Get(name)
	if err != nil {
	...
	job := *sharedJob
	// set kind for VideoAnalyticsJob in case that the kind is None
	...
	if failed > 0 {
		...
		failureMessage = "the worker of VideoAnalyticsJob failed"
	}

	if jobFailed {
		job.Status.Conditions = append(job.Status.Conditions, NewJobCondition(sednav1.VideoAnalyticsJobCondFailed, failureReason, failureMessage))
		job.Status.Phase = sednav1.VideoAnalyticsJobFailed
		...
		if complete {
			job.Status.Conditions = append(job.Status.Conditions, NewJobCondition(sednav1.VideoAnalyticsJobCondCompleted, "", ""))
			now := metav1.Now()
			job.Status.CompletionTime = &now
			c.recorder.Event(&job, v1.EventTypeNormal, "Completed", "VideoAnalyticsJob completed")
			job.Status.Phase = sednav1.VideoAnalyticsJobSucceeded
		} else {
			job.Status.Phase = sednav1.VideoAnalyticsJobRunning
		}
	}
	...
	if job.Status.Active != active || job.Status.Succeeded != succeeded || job.Status.Failed != failed || len(job.Status.Conditions) != conditions || job.Status.Phase != phase {
		...
		if jobFailed && !IsJobFinished(&job) {
			// returning an error will re-enqueue VideoAnalyticsJob after the backoff period
			return forget, fmt.Errorf("failed pod(s) detected for VideoAnalyticsJob key %q", key)
		}
...
func NewJobCondition(conditionType sednav1.VideoAnalyticsJobConditionType, reason, message string) sednav1.VideoAnalyticsJobCondition {
	return sednav1.VideoAnalyticsJobCondition{
		Type:               conditionType,
...
func (c *Controller) updateJobStatus(job *sednav1.VideoAnalyticsJob) error {
	jobClient := c.client.VideoAnalyticsJobs(job.Namespace)
...
func IsJobFinished(j *sednav1.VideoAnalyticsJob) bool {
	for _, c := range j.Status.Conditions {
		if (c.Type == sednav1.VideoAnalyticsJobCondCompleted || c.Type == sednav1.VideoAnalyticsJobCondFailed) && c.Status == v1.ConditionTrue {
			return true
		}
	}
	return false
...
func (c *Controller) createJob(job *sednav1.VideoAnalyticsJob) (active int32, err error) {
	active = 0
	...
	workerParam.WorkerType = "videoanalytics"
	...
	// create videoanalytics worker pod based on configured parameters
	_, err = runtime.CreatePodWithTemplate(c.kubeClient, job, &job.Spec.Template, &workerParam)
	if err != nil {
		return active, fmt.Errorf("failed to create videoanalytics worker: %w", err)
	}

	// Create edgemesh service for videoanalytics job
	_, err = runtime.CreateEdgeMeshService(c.kubeClient, job, VideoAnalyticsWorker, VideoAnalyticsPort)
	if err != nil {
		return active, fmt.Errorf("Failed to create edgemesh service: %w", err)
	}
...
// New creates a new video analytics job controller that keeps the relevant pods
// in sync with their corresponding VideoAnalyticsJob objects.
func New(cc *runtime.ControllerContext) (runtime.FeatureControllerI, error) {
	cfg := cc.Config

	podInformer := cc.KubeInformerFactory.Core().V1().Pods()

	jobInformer := cc.SednaInformerFactory.Sedna().V1alpha1().VideoAnalyticsJobs()
